...
def to_fixed_size_bag(bag: torch.Tensor, bag_size: int = 512) -> Tuple[torch.Tensor, int]:
    ...
    return zero_padded, min(bag_size, len(bag)), bag_idxs
...
#     model = MILModel(n_feats=768, n_out=512)
#     inputs = torch.randn(3, 10, 768)
#     #3是batch大小，10是patch的数量，768是每个patch的feature
#     lens = torch.tensor([7, 4, 6])
#     #lens是对应inputs中batch的，每一个sample中patch的数量，因为inputs是用0 padding了，将所有的样本
