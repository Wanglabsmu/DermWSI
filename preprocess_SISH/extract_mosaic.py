...
def local_binary_pattern_hist(img_imp):
    ...
    """
    lbp = local_binary_pattern(np.array(img_imp), 8, 1, 'ror')
...
if __name__ == "__main__":
    ...
    with open(os.path.join(script_dir,"checkpoints/trash_lgrlbp.pkl"), 'rb') as handle:
    ...
    pool = mp.Pool(8)
    for slide_to_process in [args.slide]: #glob.glob(os.path.join(args.slide_patch_path, "*")):
        slide_id = os.path.basename(slide_to_process).replace(".ndpi", "").replace("svs","")
        if slide_id in ignore_slide_id:
            continue
        # print("Proceessing {}/{} {}:".format(progress, total, slide_id))
        print(".................... extract mosaic ....................")
        # print(slide_to_process)
        # if slide_id + ".h5" in os.listdir(os.path.join(args.save_path, 'coord')):
        #     print("skip", slide_id)
        #     progress += 1
        #     continue
        t_start = time.time()

        patch_path = os.path.join(args.slide_patch_path, "{}.h5".format(slide_id))
        ...
        save_name = os.path.join(args.save_path, 'coord', slide_id + ".h5")
        with h5py.File(save_name, 'w') as hf:
            hf.create_dataset("coords", data=mosaic)
        # print("Proceesing {} takes:{}".format(slide_id, time.time() - t_start))
        # print("")
        progress += 1
    pool.close()
